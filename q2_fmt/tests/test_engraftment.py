...
class TestPeds(TestBase):
    ...
    def test_check_no_donors(self):
        ...
        with self.assertRaisesRegex(KeyError, 'Missing references for'
                           ' the associated sample data. Please make sure that'
                           ' all samples with a timepoint value have an'
                           ' associated reference. IDs where missing'
    ...
    def test_check_imcomplete_timepoints(self):
        metadata_df = pd.DataFrame({
            'id': ['sample1', 'sample2', 'sample3', 'sample4',
                   'donor1', 'donor2'],
    ...
    def test_check_incorrect_group_column_name(self):
        ...
        table_df = pd.DataFrame({
            ...
            'Feature2': [ 1, 1,1,1,1,1],
            'Feature3': [ 0, 0,1,1,1,1]}).set_index('id')
        with self.assertRaisesRegex(KeyError,".*finding time in the metadata" ):
            df = sample_peds(table = table_df, metadata = metadata,
                                    time_column = "time",
                                    reference_column= "Ref", 
                                    subject_column= "subject",
                                    drop_incomplete_subjects = True)

    def test_check_incorrect_suject_column_name(self):
        metadata_df = pd.DataFrame({
            'id': ['sample1', 'sample2', 'sample3', 'sample4',
                   'donor1', 'donor2'],
            'Ref': ['donor1','donor1','donor1','donor2',float("Nan"),
